...
import { FaSignOutAlt, FaChevronDown, FaChevronUp, FaSyncAlt } from 'react-icons/fa';
...
const AdminDashboard = ({ token, onLogout, role }) => {
  ...
  const handleAssignReport = async (reportId) => {
    ...
    try {
      ...
      sessionStorage.setItem('admin_reports', JSON.stringify(reports.map(report => 
        report.id === reportId ? { ...report, assigned_supervisor: selectedSupervisor } : report
      )));
                            ...
                            >
                              ...
                              <td className="p-3">
                                <button
                                  ...
                                  className="bg-azul text-blanco px-2 py-1 rounded hover:bg-azul/80 text-sm font-medium transition"
                                >
                                  Asignar
                                </button>
                              </td>
