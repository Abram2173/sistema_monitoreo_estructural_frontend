...
const ReportList = ({ token, key }) => {
    ...
    const fetchReports = useCallback(async () => {
        try {
            ...
            const response = await axios.get(`${BASE_URL}/api/reports`, {
                ...
                params: { assigned_supervisor: token.email || 'supervisor1@example.com' }, // Ajusta según tu lógica
                timeout: 10000,
            });
            console.log('Reportes recibidos:', response.data);
            setReports(response.data);
            // Actualizar last_activity al cargar reportes
            await axios.post(`${BASE_URL}/api/update-activity`, {}, {
                headers: { Authorization: `Bearer ${token}` },
            });
            console.log('Actividad del supervisor actualizada al cargar reportes');
            setLoading(false);
        } catch (err) {
    ...
    const handleAction = async (reportId, status) => {
        try {
            ...
            // Actualizar last_activity al realizar una acción
            await axios.post(`${BASE_URL}/api/update-activity`, {}, {
                headers: { Authorization: `Bearer ${token}` },
            });
            console.log('Actividad actualizada tras acción');
        } catch (err) {
    ...
    const handleDownloadImage = async (imageUrl, reportId, imageNumber) => {
        try {
            ...
            // Actualizar last_activity al descargar
            await axios.post(`${BASE_URL}/api/update-activity`, {}, {
                headers: { Authorization: `Bearer ${token}` },
            });
            console.log('Actividad actualizada tras descargar imagen');
        } catch (err) {
    ...
    const handleAnalyze = async (reportId, imageUrl) => {
        try {
            ...
            // Actualizar last_activity al analizar
            await axios.post(`${BASE_URL}/api/update-activity`, {}, {
                headers: { Authorization: `Bearer ${token}` },
            });
            console.log('Actividad actualizada tras analizar');
        } catch (err) {
